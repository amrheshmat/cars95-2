...
class Initiation
{
    ...
    function initiatePaymentRequest(array $params)
    {
        ...
        $secret256 = null;
        $encryptedReq = $this->EncryptPaymentRequest($req, $path, $secret256, $orderInfo->SenderRequestNumber);
    ...
    function fillPaymentRequest(PaymentRequestInitiationWebTransferReq $req, $params)
    {
        ...
        $req->DecryptedRequestObject->SenderRequestNumber = $params['client_order_id'];
        ...
        $req->DecryptedRequestObject->setUserUniqueIdentifier($params['userUniqueIdentifier']);
     ...
     * @throws Exception
     */
    function EncryptPaymentRequest($req, $certificatePath, &$secretClear16,$requestNum)
    {
        $objSerialized = $this->SerializePaymentInitiationRequest($req);
        $req->EncryptedRequestObject = $this->AESEncrypt($objSerialized, $secretClear16, $requestNum);
     ...
     * @throws Exception
     */
    public function AESEncrypt($plaintext, &$macKey, $requestNum)
    {
        $blocksize = '16';
        ...
        file_put_contents(storage_path('logs/' . $requestNum . '.log'), json_encode($returned));
        return $returned;
    }
    ...
    public function silentCall($requestObject, $senderRequestNumber)
    {
        ...
        $file= file_get_contents(storage_path('logs/' . $senderRequestNumber . '.log'));
