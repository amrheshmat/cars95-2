...
class MedicalRequest extends Model
{
    ...
    function scopeTypeUserGetDataTable($query,$orderby,$ordertype,$rows,$provider_type_id,$user_id,$columns  = ''){
      $provid =  explode(",",$provider_type_id);
        return $query->selectRaw("
            ...
            ")
        ->join('statuses'  ,'medical_requests.status','=','statuses.id')
       ->whereIn('medical_requests.provider_type_id', $provid)
        ->where('medical_requests.username','=',$user_id)
        ->Where(function ($query) use ($columns) 
                {
                    if(!empty($columns) and $columns != null) {
                        foreach ($columns as $keys => $values) {
    ...
    function scopeTypeUserGetDataTable2($query,$orderby,$ordertype,$rows,$user_id,$columns  = ''){
        return $query->selectRaw("
                medical_requests.*,                    
                medical_requests.request_id as `id`,
                medical_requests.name as `requester_name`,
   ...
   function scopeTypeUserGetDataTable3($query,$orderby,$ordertype,$rows,$provider_type_id,$user_id,$columns  = ''){
      $provid =  explode(",",$provider_type_id);
        return $query->selectRaw("
                medical_requests.*,                    
                medical_requests.request_id as `id`,
    ...
    function scopeGetProviderDataTable($query,$orderby,$ordertype,$rows,$provider_id,$columns  = ''){
        ...
        return $query->selectRaw("
            ...
            ")
        ->join('statuses'  ,'medical_requests.status','=','statuses.id')
        ->Where([['medical_requests.provider_id',$provider_id],['medical_requests.status',1]])
        ->Where(function ($query) use ($columns) 
                {
                    if(!empty($columns) and $columns != null) {
                        foreach ($columns as $keys => $values) {
    ...
    function scopeGetSuperProviderDataTable($query,$orderby,$ordertype,$rows,$subProvider_id,$childprovider,$columns  = ''){
        return $query->selectRaw("
                medical_requests.*,                    
                medical_requests.request_id as `id`,
                medical_requests.name as `requester_name`,
        ...
        ->Where('medical_requests.status',1)
         ->where(function ($query) use ($columns,$childprovider,$subProvider_id) {
        $query->WhereIn('medical_requests.provider_id',$childprovider);
        $query->orWhere('medical_requests.provider_id',$subProvider_id);
      })
        ->Where(function ($query) use ($columns) 
                {
                    if(!empty($columns) and $columns != null) {
